...
user_router = APIRouter(prefix="/users", tags=["Unauth User"])
...
def get_user_service(session: AsyncSession = Depends(get_session)) -> UserService:
    ...
    return UserService(
        ...
        school_repository,
        signup_token_repository,
        department_repository,
    )


@user_router.post(
    "/user/", status_code=status.HTTP_201_CREATED, response_model=UserResponse
)
...
@user_router.post(
    "/refresh/", status_code=status.HTTP_200_OK, response_model=UserLoginResponse
)
async def refresh_token(
    token=Header(), user_service: UserService = Depends(get_user_service)
):
...
async def reset_password(
    data: UserRestPasswordSchema, user_service: UserService = Depends(get_user_service)
):
...
@auth_router.get("/user/", status_code=status.HTTP_200_OK, response_model=UserResponse)
async def get_user_detail(current_user: User = Depends(security.get_current_user)):
    return current_user


@auth_router.get(
    "/signup-link/", status_code=status.HTTP_200_OK, response_model=SignupLinkResponse
)
async def generate_lecturer_signup_link(
    school_id: UUID = Query(..., description="UUID of school"),
    department_id: UUID = Query(..., description="UUID of department"),
    current_user: User = Depends(security.get_current_user),
    user_service: UserService = Depends(get_user_service),
):
    return await user_service.create_lecturer_login_token(
        school_id, department_id, current_user
    )
