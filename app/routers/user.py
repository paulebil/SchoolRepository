...
def get_user_service(session: AsyncSession = Depends(get_session)) -> UserService:
    ...
    return UserService(user_repository, user_jwt_repository, password_reset_repository, school_repository,
                       signup_token_repository, department_repository)

@user_router.post("/user/", status_code=status.HTTP_201_CREATED, response_model=UserResponse)
async def create_admin_user(data: CreateAdminUser, background_tasks: BackgroundTasks,
...
async def verify_user(data: ActivateUserSchema, background_tasks: BackgroundTasks,
                      user_service: UserService = Depends(get_user_service)):
...
async def forgot_password(data: UserForgotPasswordSchema, background_tasks: BackgroundTasks,
                          user_service: UserService = Depends(get_user_service)):
