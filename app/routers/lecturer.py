...
def get_lecturer_service(
    ...
    return LecturerService(
        ...
        reading_material_repository,
        past_paper_repository,
    )
...
async def generate_student_signup_link(
...
async def upload_reading_material(
    ...
    reading_material: UploadFile = File(...),
    current_user: User = Depends(security.get_current_user),
    lecturer_service: LecturerService = Depends(get_lecturer_service),
):
    return await lecturer_service.upload_reading_material(
        title, description, reading_material, current_user
    )
...
@lect_router.get(
    "/reading-material/",
    status_code=status.HTTP_200_OK,
    response_model=List[ReadingMaterialResponse],
)
...
async def upload_past_paper(
    ...
    year: str = Form(),
    semester: str = Form(),
    past_paper: UploadFile = File(...),
    current_user: User = Depends(security.get_current_user),
    lecturer_service: LecturerService = Depends(get_lecturer_service),
):
    return await lecturer_service.upload_past_paper(
        title, course_code, course_name, year, semester, past_paper, current_user
    )
...
@lect_router.get(
    "/past-paper/",
    status_code=status.HTTP_200_OK,
    response_model=List[PastPaperResponse],
)
async def get_all_my_past_papers(
    current_user: User = Depends(security.get_current_user),
    lecturer_service: LecturerService = Depends(get_lecturer_service),
):
