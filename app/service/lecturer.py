...
class LecturerService:
    def __init__(
        ...
        past_paper_repository: PastPaperRepository,
    ):
    ...
    async def upload_reading_material(
        self,
        title: str,
        description: str,
        reading_material: UploadFile,
        current_user: User,
    ):
        # check if user exists
        lecturer_exists = await self.user_repository.get_user_by_id(current_user.id)
        ...
        meta_data = await upload_file_to_minio(
            bucket_name, reading_material_object_name, reading_material
        )
        ...
        await self.reading_material_repository.create_reading_material(
            reading_material_to_create
        )

        return JSONResponse(
            content={"message": "Upload successful"}, status_code=status.HTTP_200_OK
        )

    async def get_all_my_reading_materials(
        self, current_user: User
    ) -> List[ReadingMaterialResponse]:
        # check if user exists
        lecturer_exists = await self.user_repository.get_user_by_id(current_user.id)
        if not lecturer_exists:
            raise HTTPException(
        ...
        content = []
        reading_materials = (
            await self.reading_material_repository.get_all_my_reading_material(
                current_user.id
            )
        )
        bucket_name = settings.MINIO_READING_MATERIAL_BUCKET_NAME
        ...
        for reading_material in reading_materials:
            ...
            file_url = await generate_presigned_url(
                bucket_name, reading_material.object_name
            )
            reading_material.file_url = file_url
            reading_material_response = ReadingMaterialResponse.model_validate(
                reading_material
            )
    ...
    async def get_reading_material_detail(
        self, reading_material_id: UUID, current_user: User
        ...
        bucket_name = settings.MINIO_READING_MATERIAL_BUCKET_NAME
        reading_material = await self.reading_material_repository.get_reading_material(
            reading_material_id
        )
        file_url = await generate_presigned_url(
            bucket_name, reading_material.object_name
    ...
    async def upload_past_paper(
        ...
        # proceed to upload the reading material
        meta_data = await upload_file_to_minio(
            bucket_name, past_paper_object_name, past_paper
        )
    ...
    async def get_all_my_past_paper(
        self, current_user: User
    ) -> List[PastPaperResponse]:
        # check if user exists
        lecturer_exists = await self.user_repository.get_user_by_id(current_user.id)
        if not lecturer_exists:
            raise HTTPException(
        ...
        if lecturer_exists.role != UserRole.LECTURER:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="User is not a lecturer. Action not allowed.",
            )
        # check if user is active
        if not lecturer_exists.is_active:
            raise HTTPException(
        ...
        content = []
        past_papers = await self.past_paper_repository.get_all_my_past_paper(
            lecturer_exists.id
        )
        bucket_name = settings.MINIO_PAST_PAPER_BUCKET_NAME
    ...
    async def get_past_paper_detail(
        self, past_paper_id: UUID, current_user: User
    ) -> PastPaperResponse:
        # check if user exists
        lecturer_exists = await self.user_repository.get_user_by_id(current_user.id)
        if not lecturer_exists:
        ...
        bucket_name = settings.MINIO_PAST_PAPER_BUCKET_NAME
        past_paper_response = await self.past_paper_repository.get_past_paper(
            past_paper_id
        )
        file_url = await generate_presigned_url(
            bucket_name, past_paper_response.object_name
        )
