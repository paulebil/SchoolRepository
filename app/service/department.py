...
class DepartmentService:
    ...
    async def create_department(
        self, data: DepartmentCreate, current_user: User
    ) -> DepartmentResponse:
        ...
        if not school_exists:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="School with this id does not exists.",
            )
        department_to_create_dict = data.model_dump(mode="json")
        if department_to_create_dict.get("school_id"):
            department_to_create_dict["school_id"] = UUID(
                department_to_create_dict["school_id"]
            )
        department_to_create = Department(**department_to_create_dict)
        created_department = await self.department_repository.create_department(
            department_to_create
        )
    ...
    async def get_departments(
        ...
        departments = await self.department_repository.get_departments_by_school_id(
            school_id
        )
        # if not departments:
        #     return []
    ...
    async def get_department(
        self, department_id: UUID, current_user: User
    ) -> DepartmentResponse:
        # check if user exists
        user_exists = await self.user_repository.get_user_by_id(user_id=current_user.id)
        if not user_exists:
        ...
        department = await self.department_repository.get_departments_by_department_id(
            department_id
        )
        if not department:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Department with this id does not exits",
            )
