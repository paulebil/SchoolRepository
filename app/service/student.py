...
class StudentService:
    def __init__(
        ...
        research_paper_repository: ResearchPaperRepository,
    ):
    ...
    async def upload_research_paper(
        ...
        if student_exists.role != UserRole.STUDENT:
            raise HTTPException(
                ...
                detail="User is not a student to create a research paper",
            )
        ...
        meta_data = await upload_file_to_minio(
            bucket_name, research_paper_object_name, research_paper
        )
        ...
        await self.research_paper_repository.create_research_paper(
            research_paper_to_create
        )
    ...
    async def get_all_my_research_paper(
        ...
        if student_exists.role != UserRole.STUDENT:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="User is not a student. Action not allowed",
            )
        # check if user is active
        if not student_exists.is_active:
            raise HTTPException(
        ...
        content = []
        research_papers = (
            await self.research_paper_repository.get_all_my_research_paper(
                student_exists.id
            )
        )
        bucket_name = settings.MINIO_RESEARCH_PAPER_BUCKET_NAME
        ...
        for research_paper in research_papers:
            ...
            file_url = await generate_presigned_url(
                bucket_name, research_paper.object_name
            )
            research_paper.file_url = file_url
            research_paper_response = ResearchPaperResponse.model_validate(
                research_paper
            )
    ...
    async def get_research_paper_detail(
        self, research_paper_id: UUID, current_user: User
        ...
        bucket_name = settings.MINIO_RESEARCH_PAPER_BUCKET_NAME
        research_paper_response = (
            await self.research_paper_repository.get_research_paper(research_paper_id)
        )
        file_url = await generate_presigned_url(
            bucket_name, research_paper_response.object_name
        )
