...
class UserService:
    def __init__(
        ...
        self.password_reset_service = PasswordResetService(
            password_reset_repository, user_repository
        )

    async def create_admin_user(
        self, data: CreateAdminUser, background_tasks: BackgroundTasks
    ) -> UserResponse:
        ...
        await UserAuthEmailService.send_account_verification_email(
            user, background_tasks
        )
        return UserResponse.model_validate(user)

    async def activate_user_account(
        self, data: ActivateUserSchema, background_tasks: BackgroundTasks
    ):
        user_to_activate = await self.user_repository.get_user_by_email(data.email)
        if not user_to_activate:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="User with this email does not exits. Link is either corrupt or not valid",
            )
        ...
        if not token_valid:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="This link is either expired or not valid.",
            )
        ...
        await UserAuthEmailService.send_account_activation_confirmation_email(
            updated_user, background_tasks
        )
        return JSONResponse(
            {
                "message": "Account is activated successfully. Check your email for confirmation."
            }
        )
    ...
    async def get_login_tokens(self, data: UserLoginSchema):
        ...
        return await security.generate_token_pair(
            user_to_login, self.user_jwt_token_repository
        )
    ...
    async def get_refresh_token(self, refresh_token: str):
        ...
        if not token_payload:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Invalid Request 1"
            )
        ...
        result = await self.user_jwt_token_repository.get_valid_user_token_with_user(
            user_id=user_id_uuid, access_key=access_key, refresh_key=refresh_key
        )

        # Unpack the tuple
        if not result:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Invalid Request 2"
            )
    ...
    async def email_forgot_password_link(
        self, data: UserForgotPasswordSchema, background_tasks: BackgroundTasks
    ):
        ...
        await self.password_reset_service.send_password_reset_email(
            user, background_tasks
        )
        return JSONResponse(
    ...
    async def reset_password(self, data: UserRestPasswordSchema):
        ...
        token_valid = await self.password_reset_service.reset_password(
            user_email, data.token, data.password
        )
        if not token_valid:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid request window.",
            )
    ...
    async def create_lecturer_login_token(
        self, school_id: UUID, department_id: UUID, current_user: User
    ) -> SignupLinkResponse:
        ...
        if not school_exists:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="School with this id does not exist",
            )

        # check if department exist and is under the school
        department_exists = await self.department_repository.get_department_by_school_id_and_department_id(
            school_id, department_id
        )
        if not department_exists:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="department with this id doesn't exist or belong to this school",
            )
        ...
        created_token = await self.signup_token_repository.create_signup_token(
            lecturer_signup_token
        )
        link = f"https://localhost:3000/signup?token={created_token.token}"
        return SignupLinkResponse(link=link)

    async def user_signup(
        self, data: SignupCreate, background_tasks: BackgroundTasks
    ) -> UserResponse:
        signup_token = await self.signup_token_repository.get_signup_token_by_token(
            data.token
        )
        if not signup_token:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Signup token does not exists.",
            )
        if signup_token.used:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Signup token already used",
            )
        ...
        await UserAuthEmailService.send_account_verification_email(
            created_lecturer, background_tasks
        )
